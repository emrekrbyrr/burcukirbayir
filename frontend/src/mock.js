// Mock data for Burcu Kırbayır's Teacher Portfolio

export const teacherInfo = {
  name: "Burcu Kırbayır",
  title: "Sınıf Öğretmeni",
  bio: "Çocukların öğrenme yolculuğunda rehber ve ilham kaynağı olmayı seven, modern öğretim yöntemleriyle eğitimi dönüştüren bir sınıf öğretmeniyim. Eğlenerek öğrenmenin gücüne inanıyor, her çocuğun potansiyelini ortaya çıkarmak için teknoloji ve yaratıcılığı bir araya getiriyorum.",
  education: "Marmara Üniversitesi - Sınıf Öğretmenliği (2021)",
  email: "burcukirbayir@gmail.com",
  profileImage: "https://customer-assets.emergentagent.com/job_teacher-burcu/artifacts/gynxrrxx_profilarkaplanburcu.png"
};

export const experiences = [
  {
    id: 1,
    year: "2023 - Şu An",
    school: "Nesibe Aydın Okulları",
    location: "Kocaeli",
    position: "Sınıf Öğretmeni",
    description: "Şu anda görev yaptığım Nesibe Aydın Okulları'nda, Sınıf öğretmenliğinin yanı sıra 1. sınıflar zümre başkanlığı görevini de yürütüyorum. Uluslararası standartlarda uygulanan PYP (Primary Years Programme) müfredatıyla öğrencilerimin hem akademik hem de sosyal-duygusal gelişimlerini destekliyorum. Çocukların kendi seçimlerini yapabildikleri ders etkinlikleriyle öğrenme sürecinde özne olmalarını teşvik ediyor, böylece özgüven ve sorumluluk duygularını güçlendiriyorum. Derslerimde yapay zekâ araçları ve Web 2.0 teknolojilerini kullanarak 21. yüzyıl becerilerini kazandırmaya odaklanıyorum. Öğrenen merkezli bir yaklaşımla her öğrencinin bireysel öğrenme stiline uygun materyaller geliştiriyor, sınıfımda demokratik ve katılımcı bir öğrenme ortamı oluşturuyorum. Akademik kazanımların yanı sıra öğrencilerimin Araştırma, Düşünme, İletişim, Öz Yönetim ve Sosyal alanlardaki öğrenme yaklaşımlarını da sistemli biçimde destekliyor ve geliştiriyorum."
  },
  {
    id: 2,
    year: "2022",
    school: "Yeşilyurt Açı Koleji",
    location: "İstanbul",
    position: "Sınıf Öğretmeni",
    description: "Yeşilyurt Açı Koleji'nde görev yaptığım dönemde, oyunlaştırılmış öğrenme yöntemleri ile öğrencilerimin derse katılımını ve motivasyonunu artırmaya odaklandım. İnteraktif akıllı tahta uygulamaları, dijital hikaye anlatımı ve çeşitli eğitim teknolojisi araçlarını kullanarak derslerimi zenginleştirdim. Öğrencilerimin problem çözme becerilerini geliştirmek için STEM odaklı projeler tasarladım. Ayrıca sınıf yönetimi konusunda pozitif disiplin yaklaşımını benimseyerek, öğrencilerimle güven temelli ilişkiler kurdum."
  },
  {
    id: 3,
    year: "2021",
    school: "YÖM İlkokulu",
    location: "İstanbul",
    position: "Sınıf Öğretmeni",
    description: "Öğretmenlik kariyerime ilk adımımı attığım YÖM İlkokulu'nda, mesleki gelişimimin temellerini attım. Bu dönemde sınıf yönetimi, farklı öğrenme ihtiyaçlarına sahip öğrencilere yaklaşım ve etkili ders planlama konularında değerli deneyimler kazandım. İlk yılımda edindiğim tecrübeler, öğretmenlik mesleğine olan tutkumu daha da artırdı. Velilerle iletişim kurma, öğrenci takip sistemlerini etkin kullanma ve okul-aile işbirliğini güçlendirme konularında pratik bilgi ve beceriler edindim."
  }
];

export const teachingApproaches = [
  {
    id: 1,
    title: "Drama Etkinlikleri",
    description: "Çocukların kendilerini ifade etmelerini ve sosyal becerilerini geliştirmelerini sağlayan yaratıcı drama çalışmaları yürütüyorum.",
    icon: "theater"
  },
  {
    id: 2,
    title: "PYP Programı",
    description: "Uluslararası Bakalorya Primary Years Programme ile öğrencilerin sorgulayıcı, bilgili ve düşünen bireyler olmalarını destekliyorum.",
    icon: "globe"
  },
  {
    id: 3,
    title: "Yapay Zeka & Teknoloji",
    description: "Yapay zeka ve Web 2.0 araçlarını derslerime entegre ederek modern ve etkili öğrenme deneyimleri yaratıyorum.",
    icon: "brain"
  },
  {
    id: 4,
    title: "Oyunlaştırma",
    description: "Öğrencilerin dersin öznesi olmalarını sağlayan oyunlaştırılmış, interaktif etkinliklerle eğlenerek öğreniyoruz.",
    icon: "gamepad"
  }
];

export const blogPosts = [
  {
    id: 1,
    title: "Sınıfta Yapay Zeka Kullanımı: Yeni Nesil Öğretim",
    excerpt: "Yapay zeka araçlarının sınıf ortamında nasıl etkili kullanılabileceğini ve öğrenci katılımını nasıl artırdığını keşfedin.",
    date: "15 Mart 2024",
    category: "Teknoloji",
    image: "https://images.pexels.com/photos/4145190/pexels-photo-4145190.jpeg",
    content: "Blog içeriği yakında eklenecek..."
  },
  {
    id: 2,
    title: "Drama ile Dil Gelişimi: Pratik Örnekler",
    excerpt: "Çocukların dil becerilerini geliştirmek için drama etkinliklerinden nasıl yararlanabiliriz? İşte uygulamalı öneriler.",
    date: "8 Mart 2024",
    category: "Drama",
    image: "https://images.unsplash.com/photo-1605627079912-97c3810a11a4",
    content: "Blog içeriği yakında eklenecek..."
  },
  {
    id: 3,
    title: "PYP Programında Sorgulama Temelli Öğrenme",
    excerpt: "Öğrencilerin merak duygularını harekete geçiren sorgulama temelli öğrenme yaklaşımının püf noktaları.",
    date: "1 Mart 2024",
    category: "PYP",
    image: "https://images.unsplash.com/photo-1603354350317-6f7aaa5911c5",
    content: "Blog içeriği yakında eklenecek..."
  }
];

export const testimonials = [
  {
    id: 1,
    name: "Ayşe Yılmaz",
    role: "Veli",
    content: "Burcu öğretmen sayesinde kızım okula gitmekten çok mutlu. Drama etkinlikleri sayesinde özgüveni arttı ve kendini daha rahat ifade ediyor. Çok teşekkür ederiz!",
    rating: 5
  },
  {
    id: 2,
    name: "Mehmet Demir",
    role: "Veli",
    content: "Oğlum için aldığımız özel dersler harika geçiyor. Burcu öğretmenin kullandığı interaktif yöntemler çocuğumun öğrenme isteğini inanılmaz artırdı.",
    rating: 5
  },
  {
    id: 3,
    name: "Zeynep Kaya",
    role: "Veli",
    content: "PYP programını ne kadar etkili uyguladığını görünce çok memnun olduk. Her gün evde okulda öğrendiklerini heyecanla anlatıyor.",
    rating: 5
  },
  {
    id: 4,
    name: "Ali Öztürk",
    role: "Veli",
    content: "Teknoloji kullanımı konusunda çok başarılı. Çocuğumuz hem eğleniyor hem öğreniyor. Oyunlaştırılmış dersler çok etkili olmuş.",
    rating: 5
  }
];

export const privateLessons = {
  title: "Özel Ders Hizmetleri",
  description: "Çocuğunuzun bireysel ihtiyaçlarına özel, oyunlaştırılmış ve etkili öğrenme deneyimleri sunuyorum.",
  features: [
    "Online ve yüz yüze ders seçenekleri",
    "Bireysel öğrenme planı",
    "İnteraktif ve oyunlaştırılmış içerik",
    "Teknoloji destekli öğretim",
    "Düzenli ilerleme raporları",
    "Esnek ders programı"
  ],
  subjects: [
    "Türkçe",
    "Matematik",
    "Fen Bilimleri",
    "Okuma-Yazma Gelişimi",
    "Ödev Desteği"
  ]
};